...
在线性回归中，为了求得参数的最优值，一般采用梯度下降和正规方程(Normal Equation)两种方式，相比梯度下降采用多次迭代逼近的方式，正规方程采用矩阵运算直接求解参数$J(\theta)$。假设函数为 $h_\theta (x) = \theta_0 + \theta_1 x_1 + \theta_2 x_2 + \theta_3 x_3 + \cdots + \theta_n x_n$，向量表示为$X*\theta = Y$，利用矩阵运算求出 $\theta$，即：
...
| 需要多次迭代             | 一次运算得出              |
| 特征值范围相差过大需要特征缩放 | 不需要 |
| 特征数量$n$很大也能适应 | 矩阵逆的计算时间复杂度为 $O(n3)$，通常来说当$n<10000$时可以接受 |
| 适用各种模型 | 只适用于线性模型 |

如果$(X^T X)$不可逆，即$(X^T X)^{-1}$不存在，一般要考虑一下两者情况：

（1） 存在冗余特征，一些特征存在线性依赖。

（2） 特征数量太多。例如$(m<n)$，对于小样本数据使用正则化。
